...
def calculate_indicator(
    ...
    if name.startswith("ATR"):
        atr = indicators.atr(high, low, close, length=config["window"], mamode=config["kind"])
        ...
        bb = indicators.bbands(close, length=config["window"], std=config["std"],
                               mamode=config["kind"]).iloc[:, -2:]  # type: ignore
        bb.columns = ["BBB", "BBP"]
        return bb[config["output"]]  # type: ignore
    elif name.startswith("MACD"):
        prices = df[config["price"]]
        macd = indicators.macd(prices, fast=config["short_window"],
                               slow=config["long_window"], signal=config["signal_window"])  # type: ignore
        ...
        dc = indicators.donchian(
            high, low, lower_length=config["window"], upper_length=config["window"])
        dcr = pd.DataFrame()
        for col, ser in dc.items():  # type: ignore
            dcr[col] = ser
        dcr.columns = ["LowDC", "MidDC", "UpDC"]
        return dcr[config["output"]]
    elif name.startswith("ERP"):  # Elder Ray Power
        ...
        mi = indicators.massi(df.high, df.low, fast=config["short_window"], slow=config["long_window"])
        ...
        ret_skew = (close - close.shift()).rolling(config["window"]).skew().rename(f"ret_skew_{config['window']}")
        return ret_skew
    elif name.startswith("ret_kurt_"):
        ret_kurt = (close - close.shift()).rolling(config["window"]).kurt().rename(f"ret_kurt_{config['window']}")
        return ret_kurt
    elif name.startswith("ret_std_"):
        ret_std = (close - close.shift()).rolling(config["window"]).std().rename(f"ret_std_{config['window']}")
        return ret_std
    elif name.startswith("ret_mean_"):
        ret_mean = (close - close.shift()).rolling(config["window"]).mean().rename(f"ret_mean_{config['window']}")
        return ret_mean
...
def create_time_features(
    ...
    if minute:
        time_features = pd.concat([
            time_features,
            encode_cyclic(timestamps.dt.minute.to_numpy(), col_name="minute", max_val=60)
        ], axis=1)
    if hour:
        time_features = pd.concat([
            time_features,
            encode_cyclic(timestamps.dt.hour.to_numpy(), col_name="hour", max_val=24)
        ], axis=1)
    if day:
        time_features = pd.concat([
            time_features,
            encode_cyclic(timestamps.dt.day.to_numpy(), col_name="day", max_val=timestamps.dt.days_in_month.to_numpy())
        ], axis=1)
    if day_of_week:
        time_features = pd.concat([
            time_features,
            encode_cyclic(timestamps.dt.day_of_week.to_numpy(), col_name="day_of_week", max_val=7)
        ], axis=1)
    if month:
        time_features = pd.concat([
            time_features,
            encode_cyclic(timestamps.dt.month.to_numpy(), col_name="month", max_val=12)
        ], axis=1)
...
def create_price_target(
    ...
    target = ((price - price.shift(-1)) / price.shift(-1)).rename("target")
    return target
